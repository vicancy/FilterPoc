...
namespace FilterPoc
{
    internal static class EdmClrTypeUtils
    {
        ...
        /// <param name="type">The type to get the instance type for.</param>
        /// <returns>The instance type for the <paramref name="typeReference"/> or null if no instance type exists.</returns>
        public static Type GetPrimitiveInstanceType(IEdmPrimitiveType type, bool isNullable)
        {
            if (type == null)
            {
                throw new ArgumentNullException("type");
        ...
        public static Type GetPrimitiveClrType(EdmPrimitiveTypeKind typeKind, bool isNullable)
        {
            switch (typeKind)
            {
        ...
        {
            return Assembly.GetAssembly(typeof(Connection)).GetType(typeName);
        }
